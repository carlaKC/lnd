...
func (r *RouterBackend) parseQueryRoutesRequest(in *lnrpc.QueryRoutesRequest) (
	*routing.RouteRequest, error) {
	...
	if len(in.BlindedPaymentPaths) > 0 { //nolint:nestif
		if len(in.PubKey) != 0 {
		...
		// For blinded paths, we get one set of features for the
		// relaying intermediate nodes and the final destination. We
		// don't allow the destination feature bit field for regular
		// payments to be set, as this could lead to ambiguity.
