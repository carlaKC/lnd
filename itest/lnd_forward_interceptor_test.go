...
var (
	// Custom record sent to the intercepting node.
	customTestKey   uint64 = 394829
	customTestValue        = []byte{1, 3, 5}

	// Custom record added by the intercepting node.
	interceptorCustomKey   uint64 = 594829
	interceptorCustomValue        = []byte{3, 2, 1}
)
...
type interceptorTestCase struct {
	...
	outgoingCustomRecord map[uint64][]byte
}
...
func testForwardInterceptorBasic(ht *lntest.HarnessTest) {
	...
	// HTLC that is forwarded on by Bob in the case of resumes.
	...
	for _, tc := range testCases {
		...
		err := interceptor.Send(&routerrpc.ForwardHtlcInterceptResponse{
			...
			UpdateAddCustomRecords: tc.outgoingCustomRecord,
		})
		require.NoError(ht, err, "failed to send request")

		// If the HTLC was resumed by bob, assert that Carol receives
		// the HTLC with any additional custom records provided.
		...
		incomingCustom := carolHTLC.IncomingUpdateAddCustomRecords[interceptorCustomKey]
		require.Equal(ht, interceptorCustomValue, incomingCustom)

		// Once we've checked this value, just resume so that the HTLC
		// will proceed as before.
		carolInterceptor.Send(&routerrpc.ForwardHtlcInterceptResponse{
...
func (c *interceptorTestScenario) prepareTestCases() []*interceptorTestCase {
	...
	cases := []*interceptorTestCase{
		...
		},
		{
			...
			outgoingCustomRecord: map[uint64][]byte{
				interceptorCustomKey: interceptorCustomValue,
			},
		},
		{
			amountMsat: 1000, shouldHold: false,
