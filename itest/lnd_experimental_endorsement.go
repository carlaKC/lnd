...
func testEndorsement(ht *lntest.HarnessTest, aliceEndorse bool) {
	...
	if aliceEndorse {
		...
		sendReq.FirstHopCustomRecords = map[uint64][]byte{
			lnwire.ExperimentalEndorsementType: expectedValue,
		}
	}
...
func validateEndorsedAndResume(ht *lntest.HarnessTest,
	...
	if expectedRecords == 1 {
		var expType uint64 = lnwire.ExperimentalEndorsementType
		value, ok := packet.IncomingHtlcWireCustomRecords[expType]
		require.True(ht, ok)
