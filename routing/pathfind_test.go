...
// runPathFindingWithRedundantAdditionalEdges asserts that we are able to
// find paths to nodes ignoring additional edges that are already known by
// self node.
func runPathFindingWithRedundantAdditionalEdges(t *testing.T, useCache bool) {
	...
	path, err := dbFindPath(
		ctx.graph, additionalEdges, ctx.bandwidthHints, &ctx.restrictParams,
		&ctx.pathFindingConfig, ctx.source, target, paymentAmt, ctx.timePref, 0,
	)

	require.NoError(t, err, "unable to find path to bob")
	require.Len(t, path, 1)

	if path[0].ChannelID != realChannelID {
		t.Fatal("additional edge for known edge wasn't ignored")
	}
}
