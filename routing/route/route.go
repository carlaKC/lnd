...
func (h *Hop) PackHopPayload(w io.Writer, nextChanID uint64,
	...
	if err := optionalBlindedField(
		h.AmtToForward == 0, isBlinded, lastHop,
	); err != nil {
	...
	if err := optionalBlindedField(
		h.OutgoingTimeLock == 0, isBlinded, lastHop,
	); err != nil {
	...
	err := validateNextChanID(nextChanID != 0, isBlinded, lastHop)
	if err != nil {
...
// - Introduction + Intermediate node: require zero values.
...
func validateNextChanID(nextChanIDSet, isBlinded, finalHop bool) error {
	switch {
	// Hops in a blinded route should not have a next channel ID set.
	case isBlinded && nextChanIDSet:
		return ErrIncorrectField
