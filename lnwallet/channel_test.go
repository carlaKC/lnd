...
func TestInvalidCommitSigError(t *testing.T) {
	...
	require.Equal(t, lnwire.ErrInvalidCommitSig, coded.ErrorCode)
}
...
func TestMaxAcceptedHTLCs(t *testing.T) {
	...
	htlc, _ := createHTLC(numHTLCs, htlcAmt)
	_, err = aliceChannel.AddHTLC(htlc, nil)
	expected := lnwire.NewCodedError(lnwire.MaxPendingHtlcsExceeded)
...
func TestMaxPendingAmount(t *testing.T) {
	...
	htlc, _ := createHTLC(numHTLCs, htlcAmt)
	_, err = aliceChannel.AddHTLC(htlc, nil)
	expected := lnwire.NewCodedError(lnwire.MaxPendingAmountExceeded)
	require.Equal(t, expected, err)
