...
func testCustomMessage(net *lntest.NetworkHarness, t *harnessTest) {
	...
	// Update Alice to accept custom protocol messages with type = 555, but
	// do not allow Bob to handle them yet.
	net.Alice.Cfg.ExtraArgs = append(
		net.Alice.Cfg.ExtraArgs,
		fmt.Sprintf(msgOverrideArg, overrideType1),
	)
	net.RestartNode(net.Alice, nil, nil)
	...
	case msg := <-messages:
		...
		// from.
		require.Equal(t.t, msgType, msg.Type, "message 1 type wrong")
		require.Equal(t.t, msgData, msg.Data, "message 1 data wrong")
		require.Equal(t.t, net.Bob.PubKey[:], msg.Peer, "message 1 "+
			"peer wrong")

	case <-time.After(defaultTimeout):
		t.t.Fatalf("alice did not receive custom message 1")
	}
	...
	net.RestartNode(net.Bob, nil, nil)
	...
	// send and Alice is configured to receiv
	ctxt, _ = context.WithTimeout(ctx, defaultTimeout)
	_, err = net.Bob.LightningClient.SendCustomMessage(
		ctxt, &lnrpc.SendCustomMessageRequest{
			Peer: net.Alice.PubKey[:],
	...
	// our message type 2 was delivered, and assert that the preceeding one
	...
	case msg := <-messages:
		// Check our type and data and (sanity) check the peer we got it
		// from.
		require.Equal(t.t, overrideType1, msg.Type, "message 2 type")
		require.Equal(t.t, msgData, msg.Data, "message 2 data")
		require.Equal(t.t, net.Bob.PubKey[:], msg.Peer, "message 2 peer")

	case <-time.After(defaultTimeout):
		t.t.Fatalf("alice did not receive custom message 2")
	}
}
