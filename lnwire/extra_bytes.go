...
func ParseAndExtractCustomRecords(allExtraData ExtraOpaqueData,
	...
	map[tlv.Type]struct{}, ExtraOpaqueData, error) {
	...
	// data map.
	parsedKnownRecords := make(map[tlv.Type]struct{}, len(knownRecords))
	for _, producer := range knownRecords {
		...
		if ok && val == nil {
			parsedKnownRecords[r.Type()] = struct{}{}
...
func MergeAndEncode(knownRecords []tlv.RecordProducer,
	...
	// Now we can sort and encode the merged records into raw bytes.
	return EncodeRecords(ProduceRecordsSorted(mergedRecords...))
}
