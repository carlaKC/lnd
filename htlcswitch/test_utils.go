...
func computePaymentRelay(link *channelLink) *record.PaymentRelayInfo {
...
func generateBlindHops(payAmt lnwire.MilliSatoshi, startingHeight uint32,
	...
	for i := len(path) - 1; i >= 0; i-- {
		...
		blindPayload := record.BlindedRouteData{
			// Padding: []byte{0x00, 0x01, 0x00, 0x00},
			ShortChannelID: &nextHop,
		...
		if !finalHop {
			...
			blindPayload.RelayInfo = computePaymentRelay(path[i])
		}
		...
		payloadBytes, err := record.EncodeBlindedRouteData(&blindPayload)
		if err != nil {
			fmt.Println("unable to encode route blinding payload")
		}
		payload.SetEncryptedData(payloadBytes)
		hops[i] = payload
	}
