...
func (b *BlindingKit) validateBlindingPoint(payloadBlinding *btcec.PublicKey,
	...
	// If both blinding points are set we don't know which role we're
	// supposed to fill for error transformation. We choose to handle the
	// error as if we are the introduction node so that we perform the
	// correct error conversion for the original sender. If we were
	// intended to be the relaying node, we've done our best and hope that
	// the error will be properly handled by the actual introduction node.
	case payloadBlindingSet && updateBlindingSet:
		return nil, 0, ErrInvalidPayload{
			Type:      record.BlindingPointOnionType,
